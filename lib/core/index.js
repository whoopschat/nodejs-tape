...
function createAuthRoute(expireIn, errRes = { code: -1, msg: 'UnAuthorized' }) {
    return function (req, res, next) {
        ...
        if (bearer) {
            token.authenticate(bearer, expireIn, config.tokenSecretKey || 'token-secret-key', (user) => {
                if (!user) {
...
function createUploadRoute(fieldName = 'upload', errRes = { code: -1, msg: 'FileNotFound' }) {
    ...
    let preUpload = multer({ storage: multerStorage }).single(fieldName);
