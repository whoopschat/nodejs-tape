...
function createUploader(dir = 'tmp', errMsg = { code: -1, msg: 'file not found' }) {
    let multerStorage = multer.diskStorage({
        destination: function (req, file, cb) {
            let uploadDir = path.join(req.config.root, req.config.assets || 'assets', dir);
    ...
    let afterUpload = function (req, res, next) {
        ...
        let filepath = path.join('/', req.config.assets || 'assets', dir, req.file.filename);
        res.json({
            url: filepath,
