...
function createAuthToken(userId) {
    return token.createToken(userId, config.tokenSecretKey || 'token-secret-key');
}
...
function createAuthRoute(errRes = { code: -1, msg: 'UnAuthorized' }) {
    return function (req, res, next) {
        ...
        if (authorization && authorization.startsWith('Bearer ')) {
            let bearer = authorization.substring('Bearer '.length);
            token.authenticate(bearer, config.tokenSecretKey || 'token-secret-key', (userId) => {
                if (!userId) {
...
function createUploadRoute(errRes = { code: -1, msg: 'FileNotFound' }) {
    ...
    let afterUpload = function (req, res, next) {
        ...
        let filepath = path.join('/', req.config.upload || 'assets/upload', dateDir, req.file.filename);
        res.send({
            url: filepath,
