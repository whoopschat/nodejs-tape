...
function authToken(token, expireIn, secretKey) {
    return token.authenticate(token, expireIn || req.config.tokenExpireIn || 30 * 86400, secretKey || config.tokenSecretKey || 'token-secret-key');
}
...
function createAuthRoute(expireIn, secretKey) {
    return function (req, res, next) {
        ...
        if (bearer) {
            req.authUser = token.authenticate(bearer, expireIn || req.config.tokenExpireIn || 30 * 86400, secretKey || config.tokenSecretKey || 'token-secret-key');
            next();
        } else {
            next();
        }
