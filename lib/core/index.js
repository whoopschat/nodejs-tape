...
function verifyToken(token, expireIn, secretKey) {
    return token.authenticate(token, expireIn || config.tokenExpireIn || 30 * 86400, secretKey || config.tokenSecretKey || 'token-secret-key');
}
...
function createUploadRoute(handle) {
    let multerStorage = multer.diskStorage({
        destination: function (req, file, cb) {
            ...
            let uploadDir = path.join(config.root, config.upload || 'assets/upload', dateDir);
